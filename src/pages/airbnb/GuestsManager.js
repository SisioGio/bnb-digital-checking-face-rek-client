...
if (token && !decodedToken ) return (
  ...
  <div className="flex justify-center items-center h-screen fixed top-0 start-0 w-full bg-green-300  bg-opacity-50 z-40">
        <h1 className="text-2xl text-gray-700 font-bold p-3 ">Booking completato, il tuo PIN verrà a breve condiviso con te</h1>
      </div>
    )
...
const BookingDetails = ({ booking }) => (
  <div className="bg-gray-50 rounded-lg p-3 shadow">
    <h2 className="text-xl font-semibold text-gray-800 mb-4">Dettagli Prenotazione</h2>
    <div className="grid grid-cols-2 gap-4 text-sm">
      <p><span className="font-medium">Codice:</span> {booking.booking_id}</p>
      <p><span className="font-medium">Ospiti:</span> {booking.number_of_guests}</p>
...
const GuestList = ({ guests, onDeleteGuest, loading, expectedGuests }) => (
  <div className="space-y-4 p-3">
    <h2 className="text-xl font-semibold text-gray-800">Lista Ospiti</h2>
    {loading ? (
...
const GuestCard = ({ guest, onDelete }) => (
  <div className={`border rounded-lg p-2 shadow-sm ${guest.sent ? 'bg-green-50' : 'bg-white'}`}>
...
const EmptyGuestCard = ({ guestNumber }) => (
  <div className="border-2 border-dashed border-gray-300 rounded-lg p-2 flex flex-col items-center justify-center  text-gray-400">
